...
function flatTexture(color: string): DataTexture {
  ...
  const t = new DataTexture(
    new Uint8Array([c.r * 255, c.g * 255, c.b * 255, 255]),
    1,
    1,
    RGBAFormat,
  );
...
export function Body({ obj }: { obj: SpaceObject }) {
  ...
  const major =
    obj.type === "planet" || obj.type === "star" || obj.id === "moon";
  const isModel = !!visual.model;

  // Orientation frames: the body's own IAU pole, or for moons the parent's.
  const frame = useMemo<PoleFrame | null>(
    () => (physical.rotation ? poleFrame(physical.rotation) : null),
    [physical.rotation],
  );
  ...
  const atmosphere = useMemo(() => {
    const preset =
      ATMOSPHERES[obj.id] ??
      (visual.atmosphere ? presetFromColor(visual.atmosphere) : null);
  ...
  useFrame((_, delta) => {
    ...
    const visible =
      isPresent(obj.id) && (selected || layerOn(obj, useSolarStore.getState()));
    ...
    // Distance LOD: a body a few pixels across needs neither a fine mesh nor its
    // atmosphere, clouds and rings (the costliest per-pixel shaders here).
    const dist = camera.position.distanceTo(pos);
    const px = (radius * lodFocal(camera)) / Math.max(dist, 1e-9);
    if (surface.current)
      surface.current.geometry = px > 24 ? SPHERE_HI : SPHERE_LO;
    if (atmoMesh.current) atmoMesh.current.visible = px > 5;
    if (clouds.current) clouds.current.visible = px > 5;
    if (rings.current) rings.current.visible = px > 1.5;
    if (atmosphere && px > 5)
    ...
    if (
      ...
      !isModel &&
      (selected ||
        camera.position.distanceTo(pos) < radius * (major ? 1e4 : 400))
    ) {
    ...
    if (isModel) {
      ...
      if (Math.abs(rate) > MAX_SPIN_DEG && !Number.isNaN(s.lastW)) {
        s.offset = wrap180(
          s.offset + Math.sign(rate) * MAX_SPIN_DEG * delta - (w - s.lastW),
        );
      } else {
      ...
      const dAngle =
        (2 * Math.PI * simSeconds) / (physical.rotationPeriodHours * 3600);
      const cap = MAX_SPIN_DEG * DEG * delta;
      spinState.current.angle += Math.max(-cap, Math.min(cap, dAngle));
      spin.current.quaternion.setFromAxisAngle(
        ECLIPTIC_NORTH,
        spinState.current.angle,
      );
    }
    lastSimTime.current = timeMs;
  ...
  const tiltRad =
    !frame && !locked && !isModel ? -(physical.axialTiltDeg ?? 0) * DEG : 0;
        ...
        {ringMat && visual.rings && (
          <group ref={rings}>
            <RingPlane
              frame={frame}
              ringMat={ringMat}
              inner={visual.rings.innerRadii * radius}
              outer={visual.rings.outerRadii * radius}
            />
          </group>
        )}
      </group>
...
/** Pixels per unit at unit distance, from the camera's vertical field of view (CSS px of the drawing buffer). */
function lodFocal(camera: object): number {
  const h = typeof window === "undefined" ? 900 : window.innerHeight;
  return h / 2 / Math.tan((((camera as { fov?: number }).fov ?? 50) * Math.PI) / 180 / 2);
}
...
function RingPlane({
  ...
  const quat = useMemo(
    () =>
      frame ? bodyQuaternion(frame, 0, new Quaternion()) : new Quaternion(),
    [frame],
  );
  return (
